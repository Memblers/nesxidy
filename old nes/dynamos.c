...
{		
	...
	*/
	...
	if (code_index)
	{
		...
		cache_code[cache_index][code_index++] = (uint8_t) ((uint16_t) &dispatch_return);
		cache_code[cache_index][code_index++] = (uint8_t) (((uint16_t) &dispatch_return) >> 8);		
	...
	if (flash_enabled)
	{
		flash_byte_program((flash_code_address + BLOCK_CONFIG_BASE + 0), flash_code_bank, pc);
		flash_byte_program((flash_code_address + BLOCK_CONFIG_BASE + 1), flash_code_bank, (pc >> 8));
	}
	...
	while (code_index < CODE_SIZE)	// for easier viewing in memory
		cache_code[cache_index][code_index++] = 0x7d;					
...
uint16_t flash_cache_select()
{
	...
	for (uint16_t i = 1; i < FLASH_CACHE_BLOCKS; i++)
	{
...
{		
	...
	{		
		...
		{					
			...
			branch_target = branch_loc;
			uint8_t test = setup_cache_branch();
			switch (test)
			{				
				case 1:	// found in same bank
				{
					...
					cache_code[cache_index][code_index+3] = (uint8_t) branch_target;
					cache_code[cache_index][code_index+4] = (uint8_t) (branch_target >> 8);
					pc += 2;
					code_index += 5;
					//break;
					cache_flag[cache_index] |= READY_FOR_NEXT;
		...
		{
			enable_interpret();
			if ((code_index + opcode_6502_rts_size + 3) < CODE_SIZE)
			{
				for (uint8_t i = 0; i < opcode_6502_rts_size; i++)
					{
						cache_code[cache_index][code_index+i] = opcode_6502_rts[i];
					}						
					pc += 2; // ??
					code_index += opcode_6502_rts_size;
					cache_flag[cache_index] |= READY_FOR_NEXT;
					return cache_flag[cache_index];
			}
			else
...
void decode_address_c(void)
{
	...
	if (msb_compare < 8)
		...
		decoded_address = 0; //decoded_address = (uint16_t) (encoded_address - ROM_OFFSET) + (uint16_t) ROM_NAME;
